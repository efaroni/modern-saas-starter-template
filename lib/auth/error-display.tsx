        ...
        <div className='ml-3 flex-1'>
          ...
          {showRetry && formatted.canRetry && onRetry && (
            <div className='mt-3'>
              <button
                onClick={onRetry}
                className={`text-sm font-medium ${getTextColor()} hover:underline focus:underline focus:outline-none`}
              >
                Try again
              </button>
            </div>
...
export function InlineAuthError({ error, className = '' }: InlineErrorProps) {
  ...
  return (
    <p className={`text-sm text-red-600 ${className}`}>{authError.message}</p>
  );
}
...
export function AuthErrorToast({ error, onDismiss }: ToastErrorProps) {
  ...
  return (
    <div className='fixed top-4 right-4 z-50 w-full max-w-sm rounded-lg border border-gray-200 bg-white shadow-lg'>
      <div className='p-4'>
        <div className='flex'>
          <div className='flex-shrink-0'>
            <svg
          ...
          <div className='ml-3 flex-1'>
            ...
            <p className='mt-1 text-sm text-gray-500'>{formatted.message}</p>
          </div>
