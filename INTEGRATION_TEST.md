...
describe('CRUD Workflow - Complete User Journey', () => {
  it('should perform complete CRUD workflow', async () => {
    await withTestTransaction(async tx => {
      ...
      // Verify deletion
      const finalGet = await request(app).get(`/api/items/${itemId}`);
...
describe('Performance Testing', () => {
  it('should handle slow database queries gracefully', async () => {
    ...
    const response = await request(app).get('/api/reports/complex').expect(504);
...
// Usage in tests
beforeEach(() => {
  mockExternalServices.openAI.onPost('/v1/chat/completions').reply(200, {
    choices: [
      {
