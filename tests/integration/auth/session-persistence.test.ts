...
import { LocalSessionStorage, MemorySessionStorage } from '@/lib/auth/session-storage'
...
describe('Auth Integration - Session Persistence', () => {
  ...
  beforeEach(async () => {
    ...
    const tempAuthService = new AuthService(authProvider, sessionStorage, tempEmailService, tempUploadService)
  ...
  describe('Cross-Service Instance Persistence', () => {
    it('should persist session across service instances', async () => {
      ...
      const firstService = new AuthService(authProvider, sessionStorage, new MockEmailService(), new MockUploadService())
      ...
      const secondService = new AuthService(authProvider, sessionStorage, new MockEmailService(), new MockUploadService())
      ...
      const profileUpdateResult = await secondService.updateUserProfile(testUser.id, {
        name: 'Updated from Second Service'
      })
      
      expect(profileUpdateResult.success).toBe(true)
      expect(profileUpdateResult.user?.name).toBe('Updated from Second Service')
      ...
      expect(updatedFirstSessionResult.user?.name).toBe('Updated from Second Service')
    ...
    it('should handle concurrent operations across instances', async () => {
      const services = Array(3).fill(null).map(() => 
        new AuthService(authProvider, sessionStorage, new MockEmailService(), new MockUploadService())
      )
  ...
  describe('Storage Layer Reliability', () => {
    it('should handle storage initialization gracefully', async () => {
      ...
      const authService = new AuthService(authProvider, freshStorage, new MockEmailService(), new MockUploadService())
    ...
    it('should handle corrupted session data gracefully', async () => {
      const authService = new AuthService(authProvider, sessionStorage, new MockEmailService(), new MockUploadService())
    ...
    it('should handle storage read failures gracefully', async () => {
      ...
      const authService = new AuthService(authProvider, failingStorage, new MockEmailService(), new MockUploadService())
  ...
  describe('Session Lifecycle Management', () => {
    ...
    it('should handle session updates during lifecycle', async () => {
      ...
      const avatarResult = await authService.uploadAvatar(testUser.id, avatarFile)
  ...
  describe('LocalStorage Integration', () => {
    ...
    it('should handle session persistence in localStorage', async () => {
      const localStorage = new LocalSessionStorage()
      const authService = new AuthService(authProvider, localStorage, new MockEmailService(), new MockUploadService())
      ...
      const newAuthService = new AuthService(authProvider, localStorage, new MockEmailService(), new MockUploadService())
