...
describe('Auth Integration - Complete User Workflows', () => {
  ...
  describe('Complete User Registration and Setup Workflow', () => {
    ...
    it('should complete full user registration and setup workflow', async () => {
      ...
      const avatarUploadResult = await authService.uploadAvatar(userId, avatarFile)
  ...
  describe('Complete Password Reset Workflow', () => {
    ...
    it('should complete full password reset workflow', async () => {
      ...
      const resetRequestResult = await authService.requestPasswordReset(testUser.email)
      ...
      const tokenVerificationResult = await authService.verifyPasswordResetToken(resetToken)
      ...
      const resetCompletionResult = await authService.resetPassword(resetToken, newPassword)
      ...
      const tokenRevalidationResult = await authService.verifyPasswordResetToken(resetToken)
      
      expect(tokenRevalidationResult.success).toBe(false)
      expect(tokenRevalidationResult.error).toBe('Invalid or expired reset token')
  ...
  describe('Complete OAuth Integration Workflow', () => {
    it('should complete OAuth sign-in workflow', async () => {
      ...
      const profileUpdateResult = await authService.updateUserProfile(oauthUserId, {
  ...
  describe('Complete Session Management Workflow', () => {
    ...
    it('should complete session lifecycle workflow', async () => {
      ...
      const newAuthService = new AuthService(authProvider, sessionStorage, emailService, uploadService)
