...
const LOAD_TEST_CONFIG = {
  ...
  ACCEPTABLE_ERROR_RATE: 0.5, // 50% (much more lenient for test env)
};
...
describe('Authentication Load Tests', () => {
  ...
  describe('Authentication Load Tests', () => {
    it('should handle concurrent login requests', async () => {
      ...
      expect(results.errorRate).toBeLessThan(
        LOAD_TEST_CONFIG.ACCEPTABLE_ERROR_RATE,
      );
      expect(results.averageResponseTime).toBeLessThan(
        LOAD_TEST_CONFIG.ACCEPTABLE_RESPONSE_TIME,
      );
      expect(results.p95ResponseTime).toBeLessThan(
        LOAD_TEST_CONFIG.ACCEPTABLE_RESPONSE_TIME * 2,
      );
  ...
  describe('Database Performance Under Load', () => {
    it('should maintain database connection pool health under load', async () => {
      ...
      expect(health.connections.active).toBeLessThanOrEqual(
        health.connections.max,
      );
...
async function runLoadTest(
  ...
  console.log(
    `Starting ${testName} load test with ${concurrentUsers} concurrent users, ${requestsPerUser} requests each`,
  );
  ...
  const workers = Array.from(
    ...
    async (_, userIndex) => {
      ...
      const delay =
        (userIndex * LOAD_TEST_CONFIG.RAMP_UP_TIME) / concurrentUsers;
