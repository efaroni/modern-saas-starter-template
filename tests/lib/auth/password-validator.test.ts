...
import { PasswordValidator, DEFAULT_PASSWORD_POLICY } from '@/lib/auth/password-validator'
...
describe('PasswordValidator', () => {
  ...
  describe('validate', () => {
    ...
    it('should reject password containing user email', () => {
      const result = validator.validate('JohnDoe123!', { email: 'john@example.com' })
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('Password should not contain your email or name')
    })

    it('should reject password containing user name', () => {
      const result = validator.validate('JohnSmith123!', { name: 'John Smith' })
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('Password should not contain your email or name')
    })
    ...
    it('should penalize passwords with repeating characters', () => {
      ...
      expect(normalResult.score).toBeGreaterThanOrEqual(repeatingResult.score - 1)
    })
  ...
  describe('getSuggestions', () => {
    it('should provide helpful suggestions for weak passwords', () => {
      ...
      expect(suggestions).toContain('Make your password longer (12+ characters recommended)')
