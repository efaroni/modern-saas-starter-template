...
describe('Avatar Upload', () => {
  ...
  describe('Avatar Upload Service', () => {
    it('should upload avatar successfully', async () => {
      ...
      expect(result.user?.image).toMatch(/^https:\/\/storage\.example\.com\/avatars\/.*\.jpg$/)
    ...
    it('should replace existing avatar when uploading new one', async () => {
      // First upload
      const firstFile = new File(['first avatar'], 'first.jpg', { type: 'image/jpeg' })
      const firstResult = await authService.uploadAvatar(testUser.id, firstFile)
      
      expect(firstResult.success).toBe(true)
      const firstImageUrl = firstResult.user?.image
      
      // Second upload should replace the first
      const secondFile = new File(['second avatar'], 'second.jpg', { type: 'image/jpeg' })
      const secondResult = await authService.uploadAvatar(testUser.id, secondFile)
      
      expect(secondResult.success).toBe(true)
      expect(secondResult.user?.image).not.toBe(firstImageUrl)
      expect(secondResult.user?.image).toMatch(/^https:\/\/storage\.example\.com\/avatars\/.*\.jpg$/)
    })
  })
  ...
  describe('Avatar Deletion', () => {
    it('should delete avatar successfully', async () => {
      ...
      const mockFile = new File(['avatar content'], 'avatar.jpg', { type: 'image/jpeg' })
      const uploadResult = await authService.uploadAvatar(testUser.id, mockFile)
