...
describe('SessionManager', () => {
  ...
  beforeEach(async () => {
    ...
    if (!result?.success || !result?.user) {
      throw new Error(
        `Failed to create test user for session tests after ${maxAttempts} attempts`,
      );
    }
  ...
  describe('destroySession', () => {
    it('should successfully destroy a session', async () => {
      ...
      let result = await sessionManager.validateSession(
        sessionToken,
        '127.0.0.1',
      );
      expect(result.valid).toBe(true);
    ...
    it('should handle destroying non-existent session gracefully', async () => {
      await expect(
        sessionManager.destroySession('non-existent-token'),
