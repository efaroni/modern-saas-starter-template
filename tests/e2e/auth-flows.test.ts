...
test.describe('Authentication E2E Tests', () => {
  ...
  test.describe('Sign Up Flow', () => {
    test('should complete successful sign up with email verification', async ({ page }) => {
      ...
      await expect(page.locator('text=Authentication & User Management')).toBeVisible();
  ...
  test.describe('Sign In Flow', () => {
    ...
    test.skip('should implement rate limiting after multiple failed attempts', async ({ page }) => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (i >= 4) { // After 5th attempt (0-indexed), should be rate limited
  ...
  test.describe('Profile Management', () => {
    test('should show profile management options when logged in', async ({ page }) => {
      const testUser = generateTestUser();
      ...
      await expect(page.locator('button[role="tab"]:has-text("Password")')).toBeVisible();
      await expect(page.locator('button[role="tab"]:has-text("Delete")')).toBeVisible();
    });
  ...
  test.describe('UI and Navigation', () => {
    ...
    test('should show development mode information', async ({ page }) => {
      ...
      await expect(page.locator('text=You can create any new user account for testing')).toBeVisible();
    });
  });
});