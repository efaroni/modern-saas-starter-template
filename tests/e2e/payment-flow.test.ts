...
test.describe('E2E Payment Flow', () => {
  ...
  test.beforeEach(async ({ browser }) => {
    ...
    page.on('response', response => {
      if (
        response.url().includes('/api/webhooks/stripe') &&
        response.status() === 401
      ) {
  ...
  test('Complete payment flow: create user, subscription payment, one-time payment', async () => {
    ...
    await test.step('Navigate to billing page', async () => {
      ...
      await expect(
        page.getByText(/billing|subscription|payment/i),
      ).toBeVisible();
    ...
    await test.step('Test subscription payment with test card', async () => {
      ...
      const subscribeButton = page
        .getByRole('button', { name: /subscribe|subscription/i })
        .first();
      ...
      // Complete payment
      await page
        .getByRole('button', { name: /pay|subscribe|complete payment/i })
        .click();

      // Wait for success or return to app
      try {
        await page.waitForURL(/\/dashboard|\/billing|\/success/, {
          timeout: 30000,
        });
        console.log('✓ Subscription payment completed');
      } catch (error) {
        console.warn(
          'Subscription payment may have webhook issues, continuing test...',
        );
      ...
      try {
        await expect(
          page.getByText(/active|subscribed|premium/i).first(),
    ...
    await test.step('Test one-time payment with test card', async () => {
      ...
      const oneTimeButton = page
        .getByRole('button', { name: /one.time|single payment|buy now/i })
        .first();
      ...
      // Complete payment
      await page
        .getByRole('button', { name: /pay|complete payment|buy now/i })
        .click();
      ...
      try {
        ...
        console.warn(
          'One-time payment may have webhook issues, continuing test...',
        );
        await page.goto('/billing-test'); // Return to billing page
      }
      ...
      try {
        await expect(
          page.getByText(/payment successful|thank you|purchased/i).first(),
        ).toBeVisible({ timeout: 10000 });
    ...
    await test.step('Test manage billing button (experimental)', async () => {
      ...
      const manageButton = page
        .getByRole('button', {
          name: /manage billing|billing portal|manage subscription/i,
        })
        .first();
      ...
      if (await manageButton.isVisible()) {
        ...
        try {
          ...
          if (currentUrl.includes('billing.stripe.com')) {
            ...
            console.log(
              '? Manage billing button clicked but portal behavior unclear',
            );
          }
        } catch (error) {
        ...
        console.log(
          '? Manage billing button not found - may not be implemented yet',
        );
      }
    });
    ...
    await test.step('Test reset user functionality (experimental)', async () => {
      ...
      const resetButton = page
        .getByRole('button', { name: /reset user|reset/i })
        .first();
      ...
      if (await resetButton.isVisible()) {
        ...
        console.log(
          '? Reset user button not found - may not be implemented yet',
        );
      }
    });
    ...
    await test.step('Test refresh status functionality (experimental)', async () => {
      ...
      const refreshButton = page
        .getByRole('button', { name: /refresh status|refresh|check status/i })
        .first();
      ...
      if (await refreshButton.isVisible()) {
        ...
        console.log(
          '? Refresh status button not found - may not be implemented yet',
        );
      }
    });

    console.log(
      'Payment flow test completed. Note: Some webhook 401 errors are expected due to ngrok URL configuration.',
    );
  });
});

// Helper function to fill Stripe test card details
async function fillStripeTestCard(
  page: Page,
  cardData: typeof STRIPE_TEST_CARD,
) {
  try {
    ...
    if (page.url().includes('checkout.stripe.com')) {
      ...
      await page
        .waitForSelector('[data-testid="card-element"]', { timeout: 10000 })
        .catch(() => {
          console.log(
            'Card element not found by test ID, trying other selectors...',
          );
        });

      // Try different approaches for Stripe Elements
      const cardFrames = page.frameLocator(
      ...
      await cardNumberFrame
        ...
        .fill(cardData.number);

      // Expiry
      const expiryFrame = cardFrames.nth(1);
      await expiryFrame
        .locator('input[name="exp-date"], input[placeholder*="expiry"]')
        .fill(cardData.expiry);
      ...
      try {
        ...
        await postalFrame
          .locator('input[name="postal"], input[placeholder*="postal"]')
          .fill(cardData.postalCode, { timeout: 2000 });
      } catch {
    ...
    try {
      ...
      console.error(
        'Could not fill card details with any method:',
